...
class NavNode(BasicNavigatorChild):
    ...
    """
    def __init__(self):
        super().__init__(node_name='nav_node') # Initialize BasicNavigator with a node name
        ...
        self.declare_parameter('goal_x', 1.1)
        self.declare_parameter('goal_y', -0.2)
        ...
        q = quaternion_from_euler(0.0, 0.0, -1.57)  # roll, pitch, yaw
...
def run_move_tests():
    ...
    moves = [
        ("drive_straight_odom", str(-0.4)),
        ("rotate_odom", str(3.14)),
...
def run_capture_basics():
    ...
    while rclpy.ok():
        if running_task == RunningTask.NONE:
            ...
            # Do an initial spin
            #if not move_node.execute_move('rotate_odom', ['1.57']):
            #    nav_node.get_logger().info('Rotate failed')

            # Command BasicNavigator superclass to go to the goal pose
            rv = nav_node.goToPose(nav_node.goal_pose)
            if not rv:
                nav_node.get_logger().error(f"goToPose returned {rv}")
            else:
                running_task = RunningTask.NAV2GOAL

        elif running_task == RunningTask.NAV2GOAL:
            if not nav_node.isTaskComplete():
                ...
                running_task = RunningTask.BACKUP  # Set to next task
                time.sleep(1)  # Optional delay before starting the next task

        elif running_task == RunningTask.BACKUP:
            run_move_tests()
            running_task = RunningTask.NONE
            break
